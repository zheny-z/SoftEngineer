...
def get_all_orders(username: str) -> List[Dict[str, Any]]:
    ...
    for order in orders:
        order_info = {
            ...
            'volume': order.charged_amount,
            'chargingTime': order.charged_time,
            'startTime': str(order.begin_time),
            'endTime': str(order.end_time),
            'chargingFee': order.charging_cost,
            'serviceFee': order.service_cost,
            'totalFee': order.total_cost,
...
def get_all_piles_status() -> List[Dict[str, Any]]:
    ...
    for pile in piles:
        pile_status = {
            'chargingPileId': str(pile.pile_id),
...
def query_report() -> List[Dict[str, Any]]:
    ...
    for pile in piles:
        ...
        pile_status = {
            ...
            'cumulativeChargingTime': pile.cumulative_charging_time,
            'cumulativeChargingAmount': pile.cumulative_charging_amount,
            'cumulativeChargingFee': cumulative_charging_earning,
            'cumulativeServiceFee': cumulative_service_earning,
            'cumulativeFee': cumulative_earning
        }
        status_list.append(pile_status)
    return status_list
